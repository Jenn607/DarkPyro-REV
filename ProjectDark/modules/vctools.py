...
async def opengc(client: Client, message: Message):
    ...
    Dark = await edit_or_reply(message, "`Turn on video chat. . .`")
...
async def joinvc(client: Client, message: Message):
    ...
    if message.from_user.id != client.me.id:
        Dark = await message.reply("`Joining video chat...`")
    else:
        Dark = await message.edit("`Joining video chat...`")
    with suppress(ValueError):
        chat_id = int(chat_id)
    try:
        await client.group_call.start(chat_id)
    except Exception as e:
        return await Dark.edit(f"**ERROR:** `{e}`")
...
async def leavevc(client: Client, message: Message):
    chat_id = message.command[1] if len(message.command) > 1 else message.chat.id
    if message.from_user.id != client.me.id:
        Dark = await message.reply("`leaving video chat...`")
    else:
        Dark = await message.edit("`Leaving video chat....`")
    with suppress(ValueError):
        chat_id = int(chat_id)
    try:
