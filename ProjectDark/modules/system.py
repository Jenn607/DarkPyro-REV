...
from ProjectDark.helpers.misc import HAPP
...
async def shutdown_bot(client: Client, message: Message):
    if BOTLOG_CHATID:
        await client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**DarkPyro-Userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(message, "**DarkPyro-Userbot Berhasil di matikan!**")
    if HAPP is not None:
...
async def logs_ubot(client: Client, message: Message):
    ...
    Dark = await edit_or_reply(message, "**Sedang Mengambil Logs Heroku**")
    with open("Logs-Heroku.txt", "w") as log:
        log.write(HAPP.get_log())
    await client.send_document(
        message.chat.id,
        "Logs-Heroku.txt",
        thumb="ProjectDark/resources/logo.jpg",
