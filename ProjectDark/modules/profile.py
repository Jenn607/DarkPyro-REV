...
async def block_user_func(client: Client, message: Message):
    ...
    if user_id == client.me.id:
        return await Dark.edit("anda stress harap segera minum obat.")
...
async def setname(client: Client, message: Message):
    ...
    if len(message.command) == 1:
        ...
        try:
            ...
            await Dark.edit(f"**Berhasil Mengubah Nama Telegram anda Menjadi** `{name}`")
...
async def set_bio(client: Client, message: Message):
    Dark = await edit_or_reply(message, "`Processing . . .`")
    if len(message.command) == 1:
        return await Dark.edit("Berikan teks untuk ditetapkan sebagai bio.")
    elif len(message.command) > 1:
        bio = message.text.split(None, 1)[1]
        try:
            await client.update_profile(bio=bio)
            await Dark.edit(f"**Berhasil Mengubah BIO anda menjadi** `{bio}`")
        except Exception as e:
            await Dark.edit(f"**ERROR:** `{e}`")
    else:
        return await Dark.edit("Berikan teks untuk ditetapkan sebagai bio.")
