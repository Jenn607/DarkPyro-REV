...
from ProjectDark.helpers.misc import HAPP, XCB
...
async def upstream(client: Client, message: Message):
    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await status.edit(
            "`[HEROKU]: Update Deploy DarkPyro-Userbot Sedang Dalam Proses...`"
        )
        ups_rem.fetch(ac_br)
...
@Client.on_message(filters.command("cupdate", ["."]) & ~filters.me)
